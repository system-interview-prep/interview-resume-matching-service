...
class TestAlgorithms(unittest.TestCase):
    ...
    def test_must_have_analyzer_detects_missing_required_skill(self):
        """Test rule-based must-have analyzer without spaCy dependency"""
        analyzer = MustHaveAnalyzer()
        resume = "Frontend developer with React and JavaScript experience."
        job = """
        Requirements:
        - Must have Python
        - Must have Docker
        - React experience is useful
        """

        result = analyzer.process_single(resume, job)

        self.assertEqual(result['algorithm'], 'must_have')
        self.assertFalse(result['details']['must_have_ok'])
        self.assertIn('python', result['details']['missing_must_have'])
        self.assertIn('docker', result['details']['missing_must_have'])
        self.assertLess(result['score'], 1.0)

    def test_must_have_analyzer_passes_required_skills(self):
        """Test must-have analyzer passes when required skills are present"""
        analyzer = MustHaveAnalyzer()
        resume = "Backend engineer with 3 years of Python, Docker, and PostgreSQL experience."
        job = "Requirements: Must have Python, Docker, PostgreSQL and 2+ years of experience."
